...
function ChatInner(){
  ...
  if(!key)return null;
  return <main className="flex min-h-[100dvh] flex-col bg-[#05070b]"><header className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-3 sm:px-6 sm:py-4 md:px-8"><div className="min-w-0"><div className="flex items-center gap-2"><span className="text-lg">🔐</span><span className="font-semibold">Private room</span></div><p className="truncate text-xs text-slate-500">End-to-end encrypted · {roomId}</p></div><button onClick={()=>{lock();router.replace(`/unlock?roomId=${encodeURIComponent(roomId)}`)}} className="rounded-lg border border-white/10 px-3 py-2 text-sm">Lock</button></header>
  <section className={`min-h-0 flex-1 overflow-y-auto p-3 transition duration-100 sm:p-4 md:p-8 ${blurred?"blur-xl select-none": ""}`} aria-label="Message list"><div className="mx-auto flex max-w-3xl flex-col gap-3">{visible.map(m=><div key={m.id} className={`rounded-2xl border border-white/10 px-3 py-3 sm:px-4 ${m.senderId===user?.uid?"ml-auto bg-cyan-400/10":"bg-white/[.04]"}`}><p className="whitespace-pre-wrap break-words text-sm leading-6">{m.display}</p><p className="mt-1 text-[10px] text-slate-500">{new Date(m.timestamp).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</p></div>)}<div ref={bottom}/></div></section>
  <form onSubmit={send} className="border-t border-white/10 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-4 md:px-8"><div className="mx-auto flex max-w-3xl flex-wrap items-stretch gap-2"><div className="mode-switch order-1 flex rounded-xl border border-white/10 bg-white/[.04] p-1" role="group" aria-label="Message display mode"><button type="button" onClick={()=>setRevealed(false)} className={!revealed?"mode-active":"mode-option"}>Coded</button><button type="button" onClick={()=>setRevealed(true)} className={revealed?"mode-active":"mode-option"}>Revealed</button></div><textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();void send(e)}}} rows={1} placeholder="Write a message…" className="order-2 min-h-12 min-w-0 flex-1 resize-none rounded-xl border border-white/10 bg-white/[.04] px-3 py-3 outline-none focus:border-cyan-400 sm:px-4"/><button className="order-3 shrink-0 rounded-xl bg-cyan-400 px-4 font-semibold text-slate-950 sm:px-5">Send</button></div>{error&&<p className="mx-auto mt-2 max-w-3xl text-xs text-red-300">{error}</p>}</form></main>
}
